 ...
 * orden estricto para poder que el sistema permita configurar el perierico X.
 * Lo primero y más importante es activar la señal del reloj principal hacia ese
 * elemento específico (relacionado con el periferico RCC), a esto llamaermos
...
/*
 * Funcion utilizada para cambiar el estado del pin entregado en el handler, asignado
 * el valor entregado en la variable newState
 */

void GPIO_WritePin(GPIO_Handler_t * pPinHandler, uint8_t newState){
	//Limpiamos la possicion que deseamos
	//pPinHandler->pGPIOx->ODR &=
	// Variable para hacer todo paso a(SET << pPinHandler->GPIO_PinConfig.GPIO_PinNumber + 16);
	if (newState == SET){
...
uint32_t GPIO_ReadPin(GPIO_Handler_t * pPinHandler){
	...
	uint32_t pinValue = 0;
	// Cargamos es valor del registro IDR, desplazado a derecha tantas veces como la ubicacion
	// del pin especifico
