...
void MCOConfig (pll_MCO1 *ptrPLL_Handler){
	...
	if(ptrPLL_Handler-> clk == HSI){
		...
		} else if (ptrPLL_Handler->presc == presc2) {

			RCC->CFGR &= ~RCC_CFGR_MCO1PRE_0;
			RCC->CFGR &= ~RCC_CFGR_MCO1PRE_1;
			RCC->CFGR |= RCC_CFGR_MCO1PRE_2;

		} else if (ptrPLL_Handler->presc == presc3) {
			RCC->CFGR |= RCC_CFGR_MCO1PRE_0;
			RCC->CFGR &= ~RCC_CFGR_MCO1PRE_1;
			RCC->CFGR |= RCC_CFGR_MCO1PRE_2;

		} else if (ptrPLL_Handler->presc == presc4) {
			RCC->CFGR &= ~RCC_CFGR_MCO1PRE_0;
			RCC->CFGR |= RCC_CFGR_MCO1PRE_1;
			RCC->CFGR |= RCC_CFGR_MCO1PRE_2;

		} else if (ptrPLL_Handler->presc == presc5) {
			RCC->CFGR |= RCC_CFGR_MCO1PRE;
	...
	else if (ptrPLL_Handler ->clk == PLL){
		RCC->CFGR |= RCC_CFGR_MCO1;
