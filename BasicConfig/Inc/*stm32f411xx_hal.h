 ...
 *
 *  NOTA: la definicion del NVIC sera realizada al momento de describir
 ...
 * 6.3.1 (RCC_CR) hasta el 6.3.12 (RCC_APB2ENR), 6.3.17 (RCC_BDCR) y el 6.3.18 (RCC_CSR)
 *
 * NOTA: La posicion de memoria (offset) debe encajar perfectamente con la posicion de memoria indicada
 * en la hoja de datos del equipo. Observe que los elementos "reservedx" tambien estan presentes allí.
 */
typedef struct
{
 ...
 * que el nuevo elemento "RCC" queda disponible para cada clase .c (arcivo.c) que incluya este archivo
 */
...
/* Definicion de la estructura de datos que representa a cada uno de los registros que componen el
 * periferico GPIO.
 * Debidoa que el periferico GPIOx es muy simple, no es muy necesario crear la descripcion bit a bit
...
typedef struct
{
	volatile uin32_t MODER;		// port mode registre					ADDR_OFFSET:	0x00
	volatile uin32_t OTYPER;    // port output type register			ADDR_OFFSET:	0x04
	volatile uin32_t OSPEEDR;   // port output speed register			ADDR_OFFSET:	0x08
	volatile uin32_t PUPDR;		// port pull-up/pul-down register		ADDR_OFFSET:	0x0C
	volatile uin32_t IDR;		// port input data register				ADDR_OFFSET:	0x10
	volatile uin32_t ODR;		// port output data register			ADDR_OFFSET:	0x14
	volatile uin32_t BSRR;		// port bit set/reset lock register		ADDR_OFFSET:	0x18
	volatile uin32_t LCKR;		// port configuration lock register		ADDR_OFFSET:	0x1C
	volatile uin32_t AFRL;		// alternate function low register		ADDR_OFFSET:	0x20
	volatile uin32_t AFRH; 		// alternate function high register		ADDR_OFFSET:	0x24
}GPIOx_RegDef_t;
 ...
 * Debido a que son varios perifericos GPIOx, es necesario la definicion para cada uno.
 *
 * NOTA: Tener cuidado que cada elemento coincida con su respectiva direccion base.
 */
#define GPIOA			((GPIOx_RegDef_t *)GPIA_BASE_ADDR)
#define GPIOB			((GPIOx_RegDef_t *)GPIB_BASE_ADDR)
#define GPIOC			((GPIOx_RegDef_t *)GPIC_BASE_ADDR)
#define GPIOD			((GPIOx_RegDef_t *)GPID_BASE_ADDR)
#define GPIOE			((GPIOx_RegDef_t *)GPIE_BASE_ADDR)
#define GPIOH			((GPIOx_RegDef_t *)GPIH_BASE_ADDR)
...
typedef struct
{
	volatile uin32_t dummy;			//Dummy Example register		ADDR_OFFSET:	0x00
}DUMMY_RegDef_t;
*/
/* No es necesario hacer nada mas en este archivo, vara este primer proyecto*/
